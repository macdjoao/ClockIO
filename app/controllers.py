...
def update_clock(user_id, clock_id):
    ...
    if current_user.user_is_admin == 0:
        ...
        try:
            ...
            log_object = ClockLogs(clocklogs_clock_id=clock_id, clocklogs_type='PUT', clocklogs_action=f'Update clock {clock_id}')
            db.session.add(log_object)
            db.session.commit()
...
def create_clock(employee_id):
    current_user = User.query.filter_by(user_id=employee_id).first()
    if current_user == None:
        return generate_response(404, 'User', {}, 'You are not a user')
    if current_user.user_is_admin == 1:
        return generate_response(400, 'User', {}, 'You dont have privileges to do this')
    else:
        body = request.get_json()
        try:
